        ...
        for protocol in protocols:
            try:
                ...
                if ip is not "127.0.53.53":
                    response = request.urlopen(protocol+domain+tld)
                    exists.append((protocol+domain+tld.lower()))
                    if outputfile is not '':
                        f.write((protocol+domain+tld.lower())+" -- "+ip+'\n')
                    print ((protocol+domain+tld.lower())+" -- "+ip)
...
def main(argv):
    ...
    try:
        ...
        print ('tld_scanner.py  [-d <domain>] [-o <outputfile>] [-i <tldfile>] [-s] (https is checked too)')
		# TODO : add examples
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-s':
